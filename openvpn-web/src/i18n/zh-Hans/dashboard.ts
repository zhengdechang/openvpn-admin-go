const translation = {
  title: "OpenVPN 客户端",
  addClient: "添加客户端",
  editClient: "编辑客户端",
  deleteClient: "删除客户端",
  clientDetails: "客户端详情",
  search: "搜索客户端...",
  noClients: "未找到客户端",
  status: {
    active: "活跃",
    inactive: "非活跃",
    pending: "待处理",
  },
  form: {
    name: "姓名",
    namePlaceholder: "输入客户端姓名",
    email: "邮箱",
    emailPlaceholder: "输入客户端邮箱",
    status: "状态",
    notes: "备注",
    notesPlaceholder: "输入关于此客户端的备注",
    save: "保存",
    cancel: "取消",
  },
  table: {
    name: "姓名",
    email: "邮箱",
    status: "状态",
    createdAt: "创建时间",
    lastConnected: "最后连接",
    ipAddress: "IP地址",
    actions: "操作",
  },
  actions: {
    edit: "编辑",
    delete: "删除",
    view: "查看",
  },
  deleteConfirm: {
    title: "删除客户端",
    message: "确定要删除此客户端吗？此操作无法撤消。",
    confirm: "删除",
    cancel: "取消",
  },
  success: {
    added: "客户端添加成功",
    updated: "客户端更新成功",
    deleted: "客户端删除成功",
  },
  error: {
    add: "添加客户端失败",
    update: "更新客户端失败",
    delete: "删除客户端失败",
    load: "加载客户端失败",
  },
  clients: {
    title: "客户",
  },
  users: {
    title: "用户",
    loadError: "加载用户或部门失败",
    downloadConfigSuccess: "下载成功",
    downloadConfigError: "下载失败",
    formRequiredFields: "请填写必填项",
    createSuccess: "创建成功",
    createError: "创建失败",
    deleteConfirm: "确定删除此用户?",
    deleteSuccess: "删除成功",
    deleteError: "删除失败",
    pageTitle: "用户管理",
    addUserButton: "新增用户",
    addUserDialogTitle: "新增用户",
    statsTotalUsers: "用户总数",
    statsTotalUsersHint: "共 {{count}} 个账号",
    statsOnline: "在线用户",
    statsOnlineHint: "当前在线 {{count}} 人",
    statsPaused: "已暂停用户",
    statsPausedHint: "已暂停 {{count}} 人",
    statsTraffic: "总流量",
    statsTrafficHint: "上行与下行总计",
    namePlaceholder: "姓名",
    emailPlaceholder: "邮箱",
    passwordPlaceholder: "密码",
    roleUser: "用户",
    roleManager: "经理",
    roleAdmin: "管理员",
    roleSuperadmin: "超级管理员",
    selectDepartmentPlaceholder: "-- 选择部门 --",
    listTitle: "用户列表",
    searchLabel: "搜索",
    searchPlaceholder: "按姓名或邮箱搜索",
    departmentFilterLabel: "部门",
    filterDepartmentAll: "全部部门",
    filterDepartmentNone: "无部门",
    statusFilterLabel: "连接状态",
    filterStatusAll: "全部状态",
    filterStatusOnline: "在线",
    filterStatusOffline: "离线",
    filterStatusPaused: "暂停",
    columnName: "姓名",
    columnEmail: "邮箱",
    columnRole: "角色",
    columnDepartment: "部门",
    columnLastConnection: "最后连接",
    columnOnlineStatus: "在线状态",
    columnCreator: "创建者",
    columnActions: "操作",
    emptyDepartment: "-",
    downloadConfigButton: "下载配置",
    osWindows: "Windows",
    osMacOS: "macOS",
    osLinux: "Linux",
    deleteButton: "删除",
    editUserDialogTitle: "编辑用户部门",
    departmentLabel: "部门",
    editingUserLabel: "编辑用户：",
    statusOnline: "在线",
    statusOffline: "离线",
    fixedIpLabel: "固定VPN IP",
    columnConnectionIp: "连接 IP",
    fixedIpPlaceholder: "例如，10.8.0.100 或者空白表示删除",
    subnetLabel: "客户端子网（可选）",
    subnetPlaceholder: "例如，10.10.120.0/23 或者空白",
    editUserDescription: "在这里对用户个人资料进行更改。完成后点击保存。",
    editUserSuccess: "用户更新成功！",
    passwordOptionalPlaceholder: "密码（可选）",
    passwordLeaveBlankPlaceholder: "留下空白以保持当前状态",
    columnAllocatedVpnIp: "VPN IP",
    roleLabel: "角色",
    editUserError: "更新用户失败。",
    downloadConfigButtonShort: "下载配置",
    columnFixedIp: "固定IP",
    statusPaused: "暂停",
    pauseButton: "暂停",
    columnAccessState: "访问状态",
    statusActive: "可访问",
    columnApproval: "审批状态",
    approvalPending: "待批准",
    approvalApproved: "已批准",
    approvalRejected: "已拒绝",
    approveButton: "批准",
    rejectButton: "拒绝",
    approveSuccess: "已批准该用户",
    approveError: "批准失败",
    rejectConfirm: "确定拒绝该用户的注册申请？",
    rejectSuccess: "已拒绝该用户",
    rejectError: "拒绝失败",
    columnSubnet: "子网",
    columnBytesReceived: "接收的字节",
    columnBytesSent: "发送的字节",
    resumeButton: "恢复",
    noResults: "没有符合筛选条件的用户。",
    resultsSummary: "共显示 {{count}} / {{total}} 位用户",
  },
  notifications: {
    pageTitle: "通知",
    markAllRead: "全部标记为已读",
    allTab: "全部",
    unreadTab: "未读",
    readTab: "已读",
    loading: "加载中...",
    noEvents: "暂无连接事件",
    columnType: "事件",
    columnUser: "用户",
    columnIPs: "IP",
    columnTime: "时间",
    columnStatus: "状态",
    connected: "已连接",
    disconnected: "已断开",
    statusRead: "已读",
    statusUnread: "未读",
    markRead: "标记已读",
  },
  logs: {
    titleServer: "日志",
    fetchServerLogsError: "获取服务器日志失败",
    usernameRequired: "请输入用户名",
    fetchClientLogsError: "获取客户端日志失败",
    serverLogsCardTitle: "服务器日志",
    noServerLogs: "暂无日志",
    clientLogsCardTitle: "客户端日志查询",
    usernamePlaceholder: "用户名",
    queryButton: "查询",
    noClientLogs: "暂无日志",
    clientConnectionLogsTitle: "客户端连接日志",
    filterByUsernamePlaceholder: "按用户名称筛选",
    searchButton: "搜索",
    showEntries: "显示 {{count}} 条记录",
    noClientConnectionLogs: "暂无客户端连接日志",
    columnUserId: "用户ID",
    columnOnlineStatus: "在线状态",
    columnOnlineDuration: "在线时长",
    columnTrafficUsage: "流量使用",
    columnLastConnectionTime: "最后连接时间",
    columnLogCreatedAt: "创建时间",
    statusOnline: "在线",
    statusOffline: "离线",
    liveColumnConnectionIp: "连接 IP",
    liveColumnBytesReceived: "收到",
    liveColumnUserId: "用户 ID",
    liveColumnVpnIp: "VPN IP",
    liveConnectionsCardTitle: "实时VPN会话",
    liveColumnOnlineDuration: "持续时间",
    noLiveConnections: "未找到活动的 VPN 会话。",
    liveColumnConnectedSince: "自连接以来",
    liveColumnBytesSent: "发送",
    fetchLiveConnectionsError: "无法获取实时VPN会话。",
  },
  server: {
    title: "服务器",
    fetchStatusError: "获取服务器信息失败",
    noPermission: "无权限访问此页面",
    statusCardTitle: "服务器状态",
    labelName: "名称：",
    labelStatus: "状态：",
    labelUptime: "运行时长：",
    labelConnected: "当前连接：",
    labelTotal: "历史总数：",
    labelLastUpdated: "最后更新时间：",
    noData: "暂无数据",
    configCardTitle: "服务器配置管理",
    updateConfigSuccess: "配置更新成功",
    updateConfigError: "配置更新失败",
    saveConfigButton: "保存配置",
    startSuccess: "启动成功",
    startError: "启动失败",
    startButton: "启动",
    stopSuccess: "停止成功",
    stopError: "停止失败",
    stopButton: "停止",
    restartSuccess: "重启成功",
    restartError: "重启失败",
    restartButton: "重启",
    controlCardTitle: "服务器控制",
    config: {
      title: "服务器配置管理",
      loading: "加载配置项...",
      fetchError: "获取配置项失败",
      saveSuccess: "配置保存成功",
      saveError: "配置保存失败",
      noChanges: "没有需要保存的更改",
      refreshButton: "刷新",
      saveAllButton: "保存所有更改",
      saving: "保存中...",
      unsavedChanges: "您有 {{count}} 项未保存的更改",
      addButton: "添加",
      unsupportedType: "不支持的类型",
      doubleClickToEdit: "双击编辑",
    },
  },
  // department: { // This seems like a duplicate or old key, 'departments' is more comprehensive.
  // title: '部门',
  // },
  departments: {
    title: "部门",
    loadError: "加载部门或用户失败",
    nameRequired: "请填写部门名称",
    createSuccess: "创建成功",
    createError: "创建失败",
    updateSuccess: "更新成功",
    updateError: "更新失败",
    deleteConfirm: "确定删除此部门？",
    deleteSuccess: "删除成功",
    deleteError: "删除失败",
    edit: "编辑",
    delete: "删除",
    pageTitle: "部门管理",
    addDepartment: "新增部门",
    addDepartmentDialogTitle: "新增部门",
    departmentNamePlaceholder: "部门名称",
    selectParentDepartment: "-- 选择上级部门 --",
    selectHead: "-- 选择负责人 --",
    cancel: "取消",
    create: "创建",
    listTitle: "部门列表",
    loading: "加载中...",
    columnName: "名称",
    columnHead: "负责人",
    columnActions: "操作",
    emptyData: "-",
    editDepartmentDialogTitle: "编辑部门",
    saveChangesButton: "保存更改",
  },
  profile: {
    updateSuccessToast: "更新成功",
    updateErrorToast: "更新失败",
    newPasswordRequired: "请输入新密码",
    passwordUpdateSuccessToast: "密码更新成功",
    passwordUpdateErrorToast: "密码更新失败",
    pageTitle: "个人资料",
    basicInfoTitle: "基本信息",
    namePlaceholder: "姓名",
    emailPlaceholder: "邮箱",
    saveButton: "保存",
    changePasswordTitle: "修改密码",
    newPasswordPlaceholder: "新密码",
    updatePasswordButton: "更新密码",
  },
  overview: {
    title: "概览",
    welcome: "欢迎使用 Aegis",
    statusCardTitle: "系统状态",
    refreshButton: "刷新",
    statRunning: "运行状态",
    statOnline: "在线客户端",
    statTotal: "历史总数",
    labelName: "服务器名称",
    labelStatus: "运行状态",
    labelUptime: "运行时长",
    labelConnected: "当前连接",
    labelTotal: "历史总数",
    labelLastUpdated: "最后更新",
    noData: "暂无数据",
    unavailable: "暂无可显示的状态信息",
    // 部署系统状态
    sysCardTitle: "系统",
    cpuCardTitle: "CPU",
    memCardTitle: "内存",
    storageCardTitle: "存储空间",
    labelVersion: "服务版本",
    labelHostname: "主机名",
    labelArch: "架构",
    labelKernel: "内核版本",
    labelGoVersion: "Go 版本",
    labelLocalTime: "本地时间",
    labelHostUptime: "运行时间",
    labelLoadAvg: "平均负载",
    labelCpuCores: "CPU 核心",
    labelCpuUsage: "已使用",
    labelMemUsed: "已使用",
    labelMemBuffers: "已缓冲",
    labelMemCached: "已缓存",
    labelSwap: "交换区",
    labelDisk: "磁盘空间",
  },
};

export default translation;
